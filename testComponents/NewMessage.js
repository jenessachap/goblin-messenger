...
const NewMessage = (props) => {
  let info = [];
  if (props.info === 'noRecipient')
    info.push(<div className='info'>You're not specifying a sender!</div>);
  if (props.info === 'noMessage')
    info.push(
      <div className='info'>
        You aren't sending any message, type something in!
      </div>
    );
  if (props.info === 'userUnknown')
    info.push(<div className='info'>That user doesn't exist!</div>);
  return (
    <div>
